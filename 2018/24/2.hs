...
main = do
  initial <- Maybe.fromJust
    . toState
    . map (map read . drop 1 . lines . Text.unpack)
    . Text.splitOn (Text.pack "\n\n")
    . Text.pack
    <$> readFile "input.txt"
  Monad.forM_ [0 ..] $ \ boost -> do
    IO.hPutStr IO.stderr $ "Trying boost " <> show boost <> " ... "
    let
      start = initial
        { stateArmies = Map.adjust (Map.map $ \ group -> group
          { groupAttackDamage = groupAttackDamage group + boost
          }) ArmyImmuneSystem $ stateArmies initial
        }
      finish = doCombat start
      immuneSystem = Map.findWithDefault Map.empty ArmyImmuneSystem $ stateArmies finish
      infection = Map.findWithDefault Map.empty ArmyInfection $ stateArmies finish
    if not (Map.null immuneSystem) && not (Map.null infection)
      then IO.hPutStrLn IO.stderr "Neither army won."
      else if Map.null immuneSystem
        then IO.hPutStrLn IO.stderr "Immune system lost."
        else do
          IO.hPutStrLn IO.stderr "Immune system won!"
          print . sum . map groupUnits $ Map.elems immuneSystem
          Exit.exitSuccess

doCombat state =
  if combatIsOver state
    then state
    else let newState = fight state in if newState == state
      then newState
      else doCombat newState
