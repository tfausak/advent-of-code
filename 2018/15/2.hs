...
main = do
  ...
  [elfPower] <- either fail (pure . map Power) (mapM Read.readEither arguments)
  input <- readFile "input.txt"
  initialArea <- either fail pure (parseArea elfPower input)
...
simulateTurn area point = case Map.lookup point (areaUnits area) of
  ...
  Just unit -> let
    ...
    in updateUnits
      ( Map.filter isAlive
      . (\ x -> if any (\ y -> unitRace y == RaceElf && not (isAlive y)) x then error "elf died" else x)
...
parseArea :: Power -> String -> Either String Area
parseArea elfPower
  = fmap (toArea elfPower)
  . traverse (pointed parseSquare)
  . withPoints
  . lines


toArea :: Power -> [(Point, Square)] -> Area
toArea elfPower =
  snd . foldr (addSquare elfPower) (Serial 1, Area Map.empty Set.empty)


addSquare :: Power -> (Point, Square) -> (Serial, Area) -> (Serial, Area)
addSquare elfPower (point, square) = case square of
  SquareElf -> addUnit RaceElf point elfPower
  SquareGoblin -> addUnit RaceGoblin point goblinPower
  SquareOpen -> id
...
addUnit :: Race -> Point -> Power -> (Serial, Area) -> (Serial, Area)
addUnit race point power (serial, area) =
  let (newSerial, unit) = makeUnit serial race power
...
makeUnit :: Serial -> Race -> Power -> (Serial, Unit)
makeUnit serial race power =
  (overSerial (+ 1) serial, Unit serial race power (Health 200))
